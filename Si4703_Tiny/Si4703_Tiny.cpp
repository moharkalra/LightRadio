...
Si4703_Tiny::Si4703_Tiny(int resetPin, int sdioPin, int sclkPin)
{
  _resetPin = resetPin;
  _sdioPin = sdioPin;
  _sclkPin = sclkPin;
...
void Si4703_Tiny::powerOn()
{
    si4703_init();
}

void Si4703_Tiny::setChannel(int channel)
{
...
int Si4703_Tiny::seekUp()
{
	return seek(SEEK_UP);
}

int Si4703_Tiny::seekDown()
{
	return seek(SEEK_DOWN);
}

void Si4703_Tiny::setVolume(int volume)
{
...
void Si4703_Tiny::readRDS(char* buffer, long timeout)
{ 
...
void Si4703_Tiny::setSeekThresh(uint16_t thresh) {
	readRegisters();
	si4703_registers[SYSCONFIG2] |= thresh; //Set volume to lowest
	updateRegisters();

}
void Si4703_Tiny::setSNRThresh(uint16_t thresh) {
	readRegisters();
	si4703_registers[SYSCONFIG3] |= thresh; //Set volume to lowest
	updateRegisters();
...
//The Tiny board has SEN pulled high, but also has SDIO pulled high. Therefore, after a normal power up
//The Si4703 will be in an unknown state. RST must be controlled
void Si4703_Tiny::si4703_init() 
{
  ...
  LRWire.begin(); //Now that the unit is reset and I2C inteface mode, we need to begin I2C
...
void Si4703_Tiny::readRegisters(){

  //Si4703 begins reading from register upper register of 0x0A and reads to 0x0F, then loops to 0x00.
  LRWire.requestFrom(SI4703, 32); //We want to read the entire register set from 0x0A to 0x09 = 32 bytes.

  while(LRWire.available() < 32) ; //Wait for 16 words/32 bytes to come back from slave I2C device
  //We may want some time-out error here

  //Remember, register 0x0A comes in first so we have to shuffle the array around a bit
  for(int x = 0x0A ; ; x++) { //Read in these 32 bytes
    if(x == 0x10) x = 0; //Loop back to zero
    si4703_registers[x] = LRWire.read() << 8;
    si4703_registers[x] |= LRWire.read();
...
byte Si4703_Tiny::updateRegisters() {

  LRWire.beginTransmission(SI4703);
  ...
  for(int regSpot = 0x02 ; regSpot < 0x08 ; regSpot++) {
    ...
    LRWire.write(high_byte); //Upper 8 bits
    LRWire.write(low_byte); //Lower 8 bits
  }

  //End this transmission
  byte ack = LRWire.endTransmission();
...
int Si4703_Tiny::seek(byte seekDirection){
  readRegisters();
...
int Si4703_Tiny::readRSSI() {
	readRegisters();
	int rssi = si4703_registers[STATUSRSSI] & 0x00FF; //Mask out everything but the lower 10 bits
